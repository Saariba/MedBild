...
def get_window_info(imagearr):  #
    ...
    for x in range(0, idxSlice):
        pic = sitk.GetArrayFromImage(imgOriginal)


def frame_image(image, low=0):
...
def give_data():  # returns an array of arrays witht the Image data
    ...
    return imgarray, patient


if __name__ == "__main__":
    frame_down = [180, 250, 250, 230, 100, 220, ]
    # idea: 1. get data(array of arrays) 2. window images 3. Map/Skale 0-257
    img, patient = give_data()
    img = frame_image(img, frame_down[patient - 1])
    print(img)
